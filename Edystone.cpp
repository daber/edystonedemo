...
void
EdystoneBeacon::releaseAlocatedURLFrame ()
{
  if (urlFrame != 0)
    {
      free (urlFrame);
...
void
EdystoneBeacon::broadcastID (uint8_t nid[10], uint8_t bid[6])
{
  releaseAlocatedURLFrame ();
  mode = MODE_UID;
...
EdystoneBeacon::Mode
EdystoneBeacon::getOperationMode ()
{
  return mode;
}
...
void
EdystoneBeacon::broadcastURL (String url)
{
  releaseAlocatedURLFrame ();
  mode = MODE_URL;
  uint8_t code = 0;
...
void
EdystoneBeacon::setBroadcastInterval (uint16_t interval_ms)
{
  //32 - 16384
  //0.625 unit ms;
